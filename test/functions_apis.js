...
describe('Functions', () => {
  describe('makeDatetimeString', () => {
    ...
    const testDate1 = new Date('March 17, 2018 03:04:00')
    const testDate2 = new Date('March 17, 2018 00:04:00')
    const testDate3 = new Date('March 1, 2018 00:04:00')
    const testDate4 = new Date('January 1, 2018 00:04:00')

    it('should return current datetime', async () => {
      const date = makeDatetimeString(testDate0).time5MinAgo
      expect(date).to.equal('2018/03/17 03:01:00')
    })
    it('should properly subtract hours when there are less than 5 minutes', async () => {
      const date = makeDatetimeString(testDate1).time5MinAgo
      expect(date).to.equal('2018/03/17 02:59:00')
    })
    it('should properly subtract days when there are less than 1 hour', async () => {
      const date = makeDatetimeString(testDate2).time5MinAgo
      expect(date).to.equal('2018/03/16 23:59:00')
    })
    it('should properly subtract months when its the first day of the month', async () => {
      const date = makeDatetimeString(testDate3).time5MinAgo
      expect(date).to.equal('2018/02/28 23:59:00')
    })
    it('should properly subtract years when its the first month of the year', async () => {
      const date = makeDatetimeString(testDate4).time5MinAgo
